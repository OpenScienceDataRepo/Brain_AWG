...
# Output dirs
OUT_DIR <- "/Volumes/Marians_SSD/ADBR_Mito/OSD-514/RNA_Seq/QC_Sex_Cov"
...
OUT_DIR <- "/Volumes/Marians_SSD/ADBR_Mito/OSD-514/RNA_Seq/DEG_Sex_Cov"
...
if (!requireNamespace("DESeq2", quietly = TRUE)) {
  stop("DESeq2 is required but not installed.")
}
...
meta$condition_group <- relevel(meta$condition_group, ref = "EARTH")
...
if (!("batch" %in% colnames(meta)) &&
    all(c("flowcell","lane") %in% colnames(meta))) {
  meta$batch <- factor(paste(meta$flowcell, meta$lane, sep = "_"))
}

use_batch <- "batch" %in% colnames(meta) && nlevels(meta$batch) > 1
use_sex   <- "sex" %in% colnames(meta) && nlevels(meta$sex) > 1
...
# dispersion plot
png(file.path(OUT_DIR, "figs", "de_01_dispersion.png"),
...
run_contrast <- function(var, levelA, levelB, tag) {
  ...
  write.csv(
    ...
    file.path(OUT_DIR, "tables",
              paste0("DE_", tag, "_sexAdjusted_unshrunk.csv")),
    row.names = FALSE
  )
  ...
  write.csv(
    ...
    file.path(OUT_DIR, "tables",
              paste0("DE_", tag, "_sexAdjusted_LFCshrunk.csv")),
    row.names = FALSE
  )

  png(file.path(OUT_DIR, "figs",
                paste0("MA_", tag, "_sexAdjusted.png")),
      1200, 900, res = 150)

  plotMA(res_shr,
         ylim = c(-4,4),
         main = paste("MA:", tag, "(sex-adjusted)"))

  dev.off()

  # summary
  sig <- sum(!is.na(res_df$padj) & res_df$padj < 0.05)
  sig_lfc <- sum(!is.na(res_df$padj) &
                 res_df$padj < 0.05 &
...
# RUN CONTRASTS
out_MG_vs_E <- run_contrast("condition_group",
                           "SPACEFLIGHT_MICROGRAVITY",
                           "EARTH",
                           "MG_vs_EARTH")

out_1G_vs_E <- run_contrast("condition_group",
                           "SPACEFLIGHT_1G",
                           "EARTH",
                           "1G_vs_EARTH")

out_MG_vs_1G <- run_contrast("condition_group",
                            "SPACEFLIGHT_MICROGRAVITY",
                            "SPACEFLIGHT_1G",
                            "MG_vs_1G")
...
writeLines(
  ...
  file.path(OUT_DIR, "tables", "DE_sexAdjusted_summary.txt")
)
...
if (!exists("OUT_DIR")) OUT_DIR <- "DEG_Sex_Cov"
dir.create(file.path(OUT_DIR, "figs"),   recursive = TRUE, showWarnings = FALSE)
dir.create(file.path(OUT_DIR, "tables"), recursive = TRUE, showWarnings = FALSE)
...
GSEA_DIR <- file.path(BASE_DIR, "GSEA_Sex_Cov")
TBL_DIR  <- file.path(BASE_DIR, "DEG_Sex_Cov", "tables")
...
load_de <- function(tag) {
  f <- file.path(TBL_DIR, paste0("DE_", tag, "_sexAdjusted_unshrunk.csv"))
